...
!ft8md added 3 uses below
  ...
  integer :: ft8_range_count,requested_threads,nthr
...
! Reading simulated wav file
           ...
           ! Reset QSO hint state once, before any worker reads it.
           if(hiscall.eq.'') then
           ...
           ! Limit the requested team to nonempty frequency ranges. The runtime
           ! may supply fewer workers (OMP_THREAD_LIMIT or nested parallelism).
           call partition_ft8_decode_range(nfa,nfb,numthreads,ft8_range_low, &
                ft8_range_high,ft8_range_count)
           if(ft8_range_count.eq.0) go to 800
           requested_threads=ft8_range_count
!$omp parallel num_threads(requested_threads) private(nthr)
!$omp single
           numthreads=1
...
!$         nthr=omp_get_thread_num()+1
           call my_ft8var%decodevar(params%nQSOProgress,nfqso, &
                ...
                mtd_worker_residual(:,nthr),mtd_worker_spectrum(:,nthr))
!$omp end parallel
           call mtd_finish(dd8)
