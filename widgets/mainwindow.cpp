...
MainWindow::MainWindow(QDir const& temp_directory, bool multiple,
  ...
  m_messageClient {new MessageClient {QApplication::applicationName (),
        ...
        this}},
  m_psk_Reporter {&m_config, product_versioned_name (m_revision)},
...
{
  // Pre-release popup and expiration lock disabled in this fork.
}
...
QString MainWindow::userAgent() {
  ...
  QString userAgent = product_user_agent (m_revision) + " " + platform;
  return userAgent;
  }
...
void MainWindow::processMessage (DecodedText const& message, Qt::KeyboardModifiers modifiers)
{
  ...
  auto cb_pair_qso = Radio::is_cb_callsign (m_baseCall)
    && (Radio::is_cb_callsign (qso_partner_base_call) || Radio::is_cb_callsign (base_call));
...
//      w34=w.at(nw-2);
    ...
    bool cb_text_for_us = cb_pair_qso
      && !message.isStandardMessage ()
      && !message.clean_string ().contains ("<")
      && w.size () >= 2
      && Radio::is_cb_callsign (w.at (0))
    ...
    if (cb_text_for_us) {
      ...
      if(message_words.at(4).contains(grid_regexp) and SpecOp::EU_VHF!=m_specOp) {
        ...
        bool word_3_is_number {false};
        auto word_3_as_number = word_3.toInt (&word_3_is_number);

        // FT8/FT4 with two CB non-standard calls cannot reliably carry signal reports.
        // Fall back to a deterministic 3-step exchange to avoid infinite looping.
        if (cb_pair_qso && word_3.isEmpty ()) {
          if (m_QSOProgress <= CALLING || m_QSOProgress < REPORT) {
            setTxMsg (2);
            m_QSOProgress = REPORT;
            ...
            m_QSOProgress = SIGNOFF;
             ...
             || (word_3_is_number && word_3_as_number == 73 && ROGERS == m_QSOProgress)
             || "RR73" == word_3
...
void MainWindow::genStdMsgs(QString rpt, bool unconditional)
{
  ...
  bool cb_nonstandard_pair = is77BitMode ()
    && SpecOp::NONE == m_specOp
    && !bMyCall
    && !bHisCall
    && Radio::is_cb_callsign (my_callsign)
    && Radio::is_cb_callsign (hisCall);
  ...
  if (eme_short_codes) {
    ...
    if (is77BitMode ()) {
      ...
      if (cb_nonstandard_pair) {
        // Two long CB calls cannot carry "CALL CALL +/-dB" reliably in FT8.
        // Use one-call reports, which are encoded as free text and preserve -07 / R-07.
        msgtype (hisCall + " " + rpt, ui->tx2);
        msgtype (hisCall + " R" + rpt, ui->tx3);
      } else {
    ...
    if (cb_nonstandard_pair) {
      msgtype (hisCall + " RR73", ui->tx4);
      if (!keepTx5) msgtype (hisCall + " 73", ui->tx5->lineEdit ());
    } else {
